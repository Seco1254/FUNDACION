...
const LIST_PAGE_URL = 'https://www.americasquarterly.org/';

export class AmericasQuarterlyScraper implements MediaScraper {
  listPageUrls = [LIST_PAGE_URL];

  extractUrls(html: string): string[] {
    const urls: string[] = [];
    const hrefRegex = /href=["'](https:\/\/www\.americasquarterly\.org\/article\/[^"']+)["']/gi;
    let match;
  ...
  private isArticleUrl(url: string): boolean {
    // Only /article/ paths, block podcasts
    if (/\/(podcasts|events|about)\//i.test(url)) return false;
    return /^https:\/\/www\.americasquarterly\.org\/article\/[\w-]+\/?$/.test(url);
  }
}
