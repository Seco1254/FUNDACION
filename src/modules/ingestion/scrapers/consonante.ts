...
const LIST_PAGE_URL = 'https://consonante.org/';

export class ConsonanteScraper implements MediaScraper {
  listPageUrls = [LIST_PAGE_URL];

  extractUrls(html: string): string[] {
    const urls: string[] = [];
    const hrefRegex = /href=["'](https:\/\/consonante\.org\/noticia\/[^"']+)["']/gi;
    let match;
  ...
  private isArticleUrl(url: string): boolean {
    return /^https:\/\/consonante\.org\/noticia\/[\w-]+\/?$/.test(url);
  }
}
